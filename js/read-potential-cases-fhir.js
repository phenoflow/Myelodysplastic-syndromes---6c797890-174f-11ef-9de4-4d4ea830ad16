...
(async () => {
  ...
  await fs.writeFile("myelodysplastic-syndromes-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("myelodysplastic-syndromes-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
